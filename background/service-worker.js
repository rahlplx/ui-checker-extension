/**
 * UI Checker DevTools Extension - Service Worker
 *
...
async function getSettings() {
  return chrome.storage.sync.get({
    ...
    autoScan: 'panel',        // 'panel' = scan when UI Checker UI opens, 'devtools' = scan when DevTools opens
  });
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.action === 'findings' && tabId) {
    ...
    chrome.scripting.executeScript({
      ...
      // Detector will post uichecker-ready, content script handles the rest
    }).catch((err) => {
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  // Connecting the sidebar is a strong signal of "user engaged with UI Checker"
