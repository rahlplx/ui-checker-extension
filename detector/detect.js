...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__UICHECKER_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip UI Checker's own elements
...
if (IS_BROWSER) {
  ...
  function showSpotlight(target) {
    ...
    if (window.__UICHECKER_CONFIG__?.spotlightBlur === false) {
  ...
  const printSummary = function(allFindings) {
    ...
    console.group(
      ...
      'color: oklch(60% 0.18 250); font-weight: bold'
    );
    for (const { el, findings } of allFindings) {
      for (const f of findings) {
        console.log(`%c${f.type || f.id}%c ${f.detail || f.snippet}`,
          'color: oklch(55% 0.18 250); font-weight: bold', 'color: inherit', el);
      }
    }
    console.groupEnd();
  };
  ...
  const scan = function() {
    ...
    overlayIndex = 0;
    const allFindings = [];
    const _disabled = EXTENSION_MODE ? (window.__UICHECKER_CONFIG__?.disabledRules || []) : [];
    const _ruleOk = (id) => !_disabled.length || !_disabled.includes(id);

    for (const el of document.querySelectorAll('*')) {
      // Skip UI Checker's own elements and any descendants (overlays, labels, banner, nav buttons)
      ...
      // Skip the UI Checker live-mode overlay (highlight, tooltip, bar, picker, toast).
      // These are inspector chrome, not part of the user's design.
      if (el.closest('[id^="uichecker-live-"]')) continue;
    ...
    // Clone the document and strip uichecker-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="uichecker-live-"]')) {
      node.remove();
    }
